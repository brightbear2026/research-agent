...
6. **交付**：`screenshot.py`（真实截图）→ `diagram_assets.py`（可选解释图）→ `render_html.py` → `evidence.py` → `qc.py --strict`（含事实就近引用、独立来源、内容禁忌、最低完整度、图片与事实漂移）终检 → `workflow_policy.py record-debt` → README。**两段式交付**：核心检查 `exit 0` 即可交付；死链/链接警告/来源时效/段落级数值来源等 advisory 项写入 `data/qc_debt.json` 作为已跟踪技术债，不阻断交付。
