...
本技能是「深度研究」流程的入口。**推荐用 `/deep-research <课题> [depth=快速|标准|深度]` 触发**（该命令含完整分阶段执行细则；本文件为可发现摘要）。

## 何时使用
用户要求就某课题做系统、广泛、深入、可验证的研究，并产出双格式报告时。

## 流程（六阶段，阶段一/二/三用 AskUserQuestion 检查点）
...
5. **合并初稿**：源标签 `[[SRC|...]]` 去重→分配 `[n]`→写 `data/citations.csv`；组装 `report/research_report.md`；跑 `qc.py` 修订。
6. **交付**：`screenshot.py`（真实截图）→ `render_html.py` → `evidence.py` → `qc.py`（含链接）终检 → README。
