...
# 输出契约（严格遵守——便于调度方合并、避免编号冲突）
1. 把章节草稿写到文件 `report/_draft_<chNN>_<slug>.md`（chNN 为两位章号，如 `_draft_ch05_market.md`）。**不要**直接改 `report/research_report.md`、`data/citations.csv` 等共享文件。
...
4. 数据点额外整理成「数据小表」附在草稿末尾（字段：指标/值/单位/时间/地域/口径/来源标签），供调度方写入 `data/source_data.csv`。
5. 需要的截图项，列出建议登记到 manifest 的行（fig_id/URL/区域/支持结论/是否原始来源）。
6. 草稿结尾写「本章结论」（含结论/主要证据/反对证据/适用条件/时间范围/置信度：高|中高|中|中低|低/决策影响）与「待补充内容」（资料缺口）。
7. 你的**最终回复**给调度方：1 段本章小结 + 源标签数量 + 写出的草稿文件路径。**不要**把整篇草稿贴回回复。
